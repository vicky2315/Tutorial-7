...
bool Shader::InitPStandard(ID3D11Device * device, WCHAR * psFilename)
{
	...
	auto pixelShaderBuffer1 = DX::ReadData(psFilename);
	HRESULT result1 = device->CreatePixelShader(pixelShaderBuffer1.data(), pixelShaderBuffer1.size(), NULL, &m_greyscalePS);
	if (result1 != S_OK)
	{
		//if loading failed. 
		return false;
	...
	matrixBufferDesc1.ByteWidth = sizeof(MatrixBufferType);
	...
	device->CreateBuffer(&matrixBufferDesc1, NULL, &m_matrixBuffer);
	...
	lightBufferDesc1.ByteWidth = sizeof(LightBufferType);
	...
	// Create the constant buffer pointer so we can access the vertex shader constant buffer from within this class.
	device->CreateBuffer(&lightBufferDesc1, NULL, &m_lightBuffer1);
	...
	device->CreateSamplerState(&samplerDesc1, &m_sampleState1);
...
bool Shader::PSetShaderParameters(ID3D11DeviceContext* context, Light* sceneLight1, ID3D11ShaderResourceView* texture2)
{
	D3D11_MAPPED_SUBRESOURCE mappedResource1;
	LightBufferType* lightPtr1;

	context->Map(m_lightBuffer1, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource1);
	lightPtr1 = (LightBufferType*)mappedResource1.pData;
	lightPtr1->ambient = sceneLight1->getAmbientColour();
	lightPtr1->diffuse = sceneLight1->getDiffuseColour();
	lightPtr1->position = sceneLight1->getPosition();
	lightPtr1->padding = 0.0f;
	context->Unmap(m_lightBuffer1, 0);
	context->PSSetConstantBuffers(1, 1, &m_lightBuffer1);	//note the first variable is the mapped buffer ID.  Corresponding to what you set in the PS
...
void Shader::EnablePShader(ID3D11DeviceContext* context)
{
	//context->IASetInputLayout(m_layout);							//set the input layout for the shader to match out geometry
	//context->VSSetShader(m_vertexShader.Get(), 0, 0);				//turn on vertex shader
	context->PSSetShader(m_greyscalePS.Get(), 0, 0);				//turn on pixel shader
	// Set the sampler state in the pixel shader.
	context->PSSetSamplers(1, 1, &m_sampleState1);
