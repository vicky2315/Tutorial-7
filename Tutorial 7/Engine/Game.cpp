...
void Game::Initialize(HWND window, int width, int height)
{
    ...
    m_Light1.setAmbientColour(0.3f, 0.3f, 0.3f, 1.0f);
    m_Light1.setDiffuseColour(1.0f, 1.0f, 1.0f, 1.0f);
    m_Light1.setPosition(2.0f, 1.0f, 1.0f);
    m_Light1.setDirection(-1.0f, -1.0f, 0.0f);
...
void Game::Update(DX::StepTimer const& timer)
{
	...
	m_view1 = m_Camera02.getCameraMatrix();
...
//	context->RSSetState(m_states->Wireframe());
    ...
    newPostProcess();
    m_sprites->Draw(m_renderTarget2->getShaderResourceView(), m_bloomRect);
    m_sprites->End();
    // Show the new frame.
...
void Game::PostProcess()
{
    ...
    m_renderTarget1->setRenderTarget(context);
    m_renderTarget1->clearRenderTarget(context, 0.5f, 0.5f, 0.5f, 0.5f);

    m_BasicShaderPair1.EnablePShader(context);
    m_BasicShaderPair1.PSetShaderParameters(context, &m_Light, m_FirstRenderPass->getShaderResourceView());

    //m_BasicModel.Render(context);
    context->OMSetRenderTargets(1, &renderTargetView, depthTargetView);

    //m_sprites->Begin();
    //m_sprites->Draw(m_renderTarget1->getShaderResourceView(), m_bloomRect);
    //m_sprites->End();
    
}

void Game::newPostProcess()
{
    auto context = m_deviceResources->GetD3DDeviceContext();
    auto renderTargetView = m_deviceResources->GetRenderTargetView();
    auto depthTargetView = m_deviceResources->GetDepthStencilView();

    auto txt = m_FirstRenderPass->getShaderResourceView();
    
    ID3D11ShaderResourceView* null[] = { nullptr, nullptr };
    m_renderTarget2->setRenderTarget(context);
    m_renderTarget2->clearRenderTarget(context, 0.5f, 0.5f, 0.5f, 0.5f);

    m_sprites->Begin(SpriteSortMode_Immediate,
        nullptr, nullptr, nullptr, nullptr,
        [=]() {
            context->PSSetShader(greycolor_ps.Get(), nullptr, 0);
            context->PSSetShaderResources(1, 2, &txt);
            context->PSSetConstantBuffers(1, 1, m_greyscalebuffer.GetAddressOf());
        });
...
void Game::RenderTexturePass1()
{
	...
	m_world1 = SimpleMath::Matrix::Identity;
	// Turn our shaders on,  set parameters
	m_BasicShaderPair.EnableShader(context);
	m_BasicShaderPair.SetShaderParameters(context, &m_world1, &m_view1, &m_projection1, &m_Light, m_texture1.Get());
	...
	m_world1 = m_world1 * newPosition;

	//setup and draw sphere
	m_BasicShaderPair.EnableShader(context);
	m_BasicShaderPair.SetShaderParameters(context, &m_world1, &m_view1, &m_projection1, &m_Light, m_texture2.Get());
	m_BasicModel2.Render(context);

	//prepare transform for floor object. 
	m_world1 = SimpleMath::Matrix::Identity; //set world back to identity
	SimpleMath::Matrix newPosition2 = SimpleMath::Matrix::CreateTranslation(0.0f, -0.6f, 0.0f);
	m_world1 = m_world1 * newPosition2;
...
void Game::CreateDeviceDependentResources()
{
    ...
    auto blob = DX::ReadData(L"colour_ps.cso");
    DX::ThrowIfFailed(device->CreatePixelShader(blob.data(), blob.size(),
        nullptr, greycolor_ps.ReleaseAndGetAddressOf()));
    {
        CD3D11_BUFFER_DESC colorBufferDesc((sizeof(ColorBufferType)+4), D3D11_BIND_CONSTANT_BUFFER);
        DX::ThrowIfFailed(device->CreateBuffer(&colorBufferDesc, nullptr,
            m_greyscalebuffer.ReleaseAndGetAddressOf()));
    }
	...
	m_BasicShaderPair.InitStandard(device, L"light_vs.cso", L"BloomExtract.cso");
    //m_BasicShaderPair1.InitPStandard(device, L"Greyscale_ps.cso");
	...
	m_renderTarget2 = new RenderTexture(device, 800, 600, 1, 2);
}
